...
#define MIN_QUANT 1
#define MAX_QUANT 50
//returns the index of the smallest rburst time in array
int small(struct process *arr, int cpuTime, int num) {
  int remaining_time = MAX_BURST+1;
  int index = -1;
  int i = 0;
  while (arr[i].arrival <= cpuTime && i < num) {
    if (arr[i].rburst < remaining_time && arr[i].rburst != 0) {
      remaining_time = arr[i].rburst;
      index = i;
    }
    i++;
  }
